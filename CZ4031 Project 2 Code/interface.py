...
class Application(ttk.Window):
    ...
    def generate_UI(self):
        ...
        self.app_label = ttk.Label(self.window_container, text="CZ4031 Project 2",
                                   font=FONT_TITLE, anchor=CENTER, background="#2C3143", foreground='white')
        self.app_label.pack(fill=tk.X, pady=[30, 30])

        # Horizontal line below title
        s.configure("Line.TSeparator", background="black")
        separator = ttk.Separator(
            self.window_container, orient='horizontal', style="Line.TSeparator")
        ...
        # Frame for left
        self.window_container_left = ttk.Frame(
            panedwindow, width=250, height=400)
        self.window_container_left.pack(fill=tk.BOTH, side=LEFT)

        # Frame for right
        self.window_container_right = ttk.Frame(
            panedwindow, width=250, height=400)
        ...
        queries_text = {
            ...
            WHERE customer.c_custkey = orders.o_orderkey 
                AND lineitem.l_orderkey = orders.o_orderkey 
                AND orders.o_orderdate < '1995-03-15'  AND c_mktsegment = 'BUILDING' 
            GROUP BY l_orderkey, o_orderdate, o_shippriority 
            ORDER BY revenue desc, o_orderdate LIMIT 20""",
            ...
            "Query 7": """SELECT o_orderdate, o_shippriority
            FROM customer, orders 
            WHERE customer.c_custkey = orders.o_orderkey 
                AND orders.o_orderdate < '1995-03-15'
                AND customer.c_mktsegment = 'BUILDING' 
            ...
            "Query 11": """SELECT o_orderdate, o_shippriority
            FROM customer, orders 
            WHERE customer.c_custkey = orders.o_orderkey 
            AND orders.o_orderdate < '1995-03-15'
            GROUP by o_orderdate, o_shippriority
            ...
            "Query 15": """SELECT C.c_custkey  
            FROM customer C, orders O 
            WHERE C.c_custkey = O.o_custkey""",

            "Query 16": """SELECT orders.o_orderkey, customer.c_custkey, lineitem.l_partkey, lineitem.l_quantity, lineitem.l_extendedprice
            ...
            "Query 17": """SELECT orders.o_orderkey, customer.c_custkey
        ...
        self.sql_container1 = ttk.Frame(
            self.window_container_left, borderwidth=0)
        self.sql_container1.pack(pady=10, fill=tk.BOTH)

        my_label = Label(self.sql_container1,
                         text="Initial Query:", font=FONT_NORMAL)
        my_label.configure(background='#2C3143', foreground='white')
        my_label.pack(padx=10, pady=10, anchor=NW, side=LEFT)

        value1 = tk.StringVar()
        self.example_query = ttk.OptionMenu(
            self.sql_container1, value1, queries_selection[0], *queries_selection)
        self.example_query.pack(padx=10, pady=10, anchor=NE, side=RIGHT)

        self.text_container1 = ttk.Frame(
            self.window_container_left, borderwidth=0)
        self.text_container1.pack()

        self.query_1 = Text(self.text_container1, width=70,
                            height=12, wrap="word")
        ...
        self.sql_container2 = ttk.Frame(
            self.window_container_left, borderwidth=0)
        self.sql_container2.pack(pady=10, fill=tk.BOTH)

        my_label = Label(self.sql_container2,
                         text="New Query:", font=FONT_NORMAL)
        my_label.configure(background='#2C3143', foreground='white')
        my_label.pack(padx=10, pady=10, anchor=NW, side=LEFT)

        value2 = tk.StringVar()
        self.example_query = ttk.OptionMenu(
            self.sql_container2, value2, queries_selection[0], *queries_selection)
        self.example_query.pack(padx=10, pady=10, anchor=NE, side=RIGHT)

        self.text_container2 = ttk.Frame(
            self.window_container_left, borderwidth=0)
        self.text_container2.pack()

        self.query_2 = Text(self.text_container2, width=70,
                            height=12, wrap="word")
        ...
        self.submit_button = ttk.Button(
            self.text_container2, text="Submit", command=self.submit_queries, bootstyle="secondary")
        self.submit_button.pack(pady=20)

        # Right window -----------------------------------------------------------------------------------

        s.configure("Custom.TNotebook", tabposition="n",
                    background="#2C3143", bordercolor="#2C3143")
        s.configure("Custom.TNotebook.Tab",
                    background="#6C788B", foreground='white')
        s.map("Custom.TNotebook.Tab", background=[
              ("selected", "#2C3143")], foreground=[("selected", "white")])

        # create the notebook with the custom style
        self.tabs_holders = ttk.Notebook(
            self.window_container_right, style="Custom.TNotebook")
        ...
        self.initial_query_plan_label = Label(
            self.initial_subframe, text="Initial Query:", font=FONT_UNDERLINE)
        self.initial_query_plan_label.configure(
            background='#2C3143', foreground='white')
        self.initial_query_plan_label.pack(
            padx=20, pady=20, expand=True, fill=BOTH)
        self.initial_query_plan_text = Text(
            self.initial_subframe, width=40, height=50, wrap="word")
        self.initial_query_plan_text.pack(
            padx=10, pady=10, expand=True, fill=BOTH)

        self.new_query_plan_label = Label(
            self.new_subframe2, text="New Query:", font=FONT_UNDERLINE)
        self.new_query_plan_label.configure(
            background='#2C3143', foreground='white')
        self.new_query_plan_label.pack(
            padx=20, pady=20, expand=True, fill=BOTH)
        self.new_query_plan_text = Text(
            self.new_subframe2, width=40, height=50, wrap="word")
        ...
        self.analysis_label = Label(
            self.analysis_container, text="What has changed and why:", font=("Helvetica", 18))
        self.analysis_label.configure(background='#2C3143', foreground='white')
        self.analysis_label.pack(pady=20)
        self.analysis_text = Text(
            self.analysis_container, width=70, height=50, wrap="word")
        self.analysis_text.pack(pady=10, padx=10, expand=True, fill=BOTH)

        self.sql_output_container_initial = ttk.Frame(
            self.tabs_holders, borderwidth=0)
        self.sql_output_container_initial.pack(fill=tk.BOTH)
        self.tabs_holders.add(
            self.sql_output_container_initial, text="Output (Initial Query)")

        self.sql_output_container_new = ttk.Frame(
            self.tabs_holders, borderwidth=0)
        self.sql_output_container_new.pack(fill=tk.BOTH)
        self.tabs_holders.add(self.sql_output_container_new,
                              text="Output (New Query)")
        ...
        self.login_window.geometry("%dx%d+%d+%d" %
    ...
    def illustrating_changes(self, initial, new):
        ...
        if isValid:
            ...
            if updated_clause == None:
                messagebox.showerror(
                    "showwarning", "SQL Queries are the same, please ensure they are different!")
                ...
                self.analysis_text.insert(
                    END, "Difference in SQL Queries: \n\n", ("title",))
                self.analysis_text.insert(END, added_analysis_text, ("body",))

                # Get Query plans from user input
                try:
                    initialPlan = preprocessor.get_query_plan(
                        self.initial_query)
                    newPlan = preprocessor.get_query_plan(self.new_query)
                except Exception as e:
                    messagebox.showerror(
                        "showwarning", "Please input a working SQL Query!")
                    return

                for widget in self.sql_output_container_initial.winfo_children():
                    widget.destroy()

                self.initial_button = ttk.Button(self.sql_output_container_initial, text="Generate Table", command=lambda: self.tableTab(
                    self.initial_query, self.sql_output_container_initial), bootstyle="secondary")
                self.initial_button.pack(pady=20, expand=True)

                for widget in self.sql_output_container_new.winfo_children():
                    widget.destroy()

                self.new_button = ttk.Button(self.sql_output_container_new, text="Generate Table", command=lambda: self.tableTab(
                    self.new_query, self.sql_output_container_new), bootstyle="secondary")
                ...
                try:
                    ...
                    # Resize Image
                    w, h = imgInitial.size
                    ratio = min(self.initial_query_plan_text.winfo_width(
                    ) / w, self.initial_query_plan_text.winfo_height() / h)
                    new_size = (int(w * ratio), int(h * ratio))
                    imgInitial = imgInitial.resize(new_size)

                    # Insert Image
                    self.initial_img = ImageTk.PhotoImage(imgInitial)
                    self.initial_query_plan_text.image_create(
                        END, image=self.initial_img)
                    ...
                    # Resize Image
                    w, h = imgNew.size
                    ratio = min(self.new_query_plan_text.winfo_width(
                    ) / w, self.new_query_plan_text.winfo_height() / h)
                    new_size = (int(w * ratio), int(h * ratio))
                    imgNew = imgNew.resize(new_size)

                    # Insert Image
                    self.new_img = ImageTk.PhotoImage(imgNew)
                    self.new_query_plan_text.image_create(
                        END, image=self.new_img)
                    ...
                    if initialPlan == newPlan:
                        ...
                        self.analysis_text.config(state="normal")
                        self.analysis_text.insert(
                            '1.0', "The SQL Queries have the same query plan!")
                        ...
                        self.analysis_text.config(state="normal")
                        self.analysis_text.insert(
                            END, "In the Initial Query:\n", ("title",))
                        joinResults, scanResults = searchFunction.searchJoin(
                            initialPlan)
                        initialJoinString = self.printJoin(
                            joinResults, scanResults, self.analysis_text)
                    ...
                    self.analysis_text.tag_configure(
                        "title", font=title_font, underline=True)
                    self.analysis_text.tag_configure("body", font=body_font)
                    self.analysis_text.config(state=DISABLED)

                except Exception as e:
                    messagebox.showerror(
                        "showwarning", "Both Queries are the same!")
    ...
    def tableTab(self, query, container):
        ...
        if (actual_output is None or (len(actual_output) == 0)):
            self.no_results = ttk.Label(container, text="No results matching the query provided.",
                                        font=FONT_BOLD, anchor=CENTER, background="#2C3143", foreground='white')
